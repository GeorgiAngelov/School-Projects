...
int main(int argc, char * argv[])
{
	...
	if ( pid == 0 ) // Child process 
	{
		...
		// Compute the Nth fibonacci number in this child
		unsigned int fibN = 10;
		int * fib = 0;			// accidentally initialized to NULL
		fib[0] = 1;
		fib[1] = 1;
		for (unsigned int n=2; n < fibN; ++n)
			fib[n] = fib[n-1] + fib[n-2];

		std::cout << "In the child (if): The "<< fibN << " fibonacci element is " << fib[fibN] << std::endl; 
